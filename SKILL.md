...
| 触发 | 一线修复 | 仍失败兜底 |
|------|---------|-----------|
| `agent_tools.py` 导入失败（缺包） | `pip install pandas`，检查 `requirements.txt` | 退回到 `local_agent_router()` 本地路由 |
| CSV 数据文件缺失 | 尝试 `integrated_selection_products.csv` → `merged_products.csv` → `final_products.csv` 依次 fallback | 告知用户"当前无法读取商品数据"，不编造 |
| 品类关键词无匹配 | 扩大搜索到三级分类+商品名称全文 | 诚实告知"该品类暂未收录，建议补充爬取" |
| LLM 后端不可用 | 依次尝试 claude→openai→deepseek | 降级到 `local_agent_router()`，无自然语言分析 |
| 用户输入无法识别场景 | 默认走 `pick` 场景 + 追问"你是想选品/清仓/比价/促销/进货？" | 展示 6 场景菜单让用户手动选 |

## 🔴 CHECKPOINT 检查点

🔴 场景识别后：先向用户确认"识别到你在做[场景名]，对吗？"，等确认再继续数据采集。

🔴 策略生成后：先展示 2-3 条核心建议摘要，等用户点头再展开完整建议书。不要直接输出大段 Markdown。

🔴 置信度 < 0.5 或覆盖率 < 10%：先说"当前数据不足以支撑高置信度建议，以下仅供参考"，再展示报告。

## 🚫 禁止行为清单

1. **禁止不读数据就推荐**：每次选品必须先执行至少一次 CSV 查询
2. **禁止编造商品信息**：找不到数据写"当前数据不支持该结论"
3. **禁止隐藏覆盖率局限**：京东评论 3.2%、慢慢买 2.4%——每次必标注
4. **禁止替用户做进货决策**：给建议标置信度，最终决策由用户下
5. **禁止场景混淆**：不同场景用不同模板，不对清仓场景给选品建议
6. **禁止泄露用户成本/库存**：敏感数据仅会话用，不写 CSV 或持久化
7. **禁止在差评中引用用户ID**：只做分类统计，不贴完整评论原文
